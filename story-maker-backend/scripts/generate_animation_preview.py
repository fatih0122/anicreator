"""
Generate Animation Style Preview Images

This script generates 5 preview images for the Animation (Disney-Pixar) style only.

Usage:
    python scripts/generate_animation_preview.py
"""
...
async def main():
    """Generate previews for Animation style"""
    print("="*60)
    print("🎨 Animation Style Preview Generator")
    print("="*60)
    print(f"\nGenerating 5 preview images for Animation (Disney-Pixar) style")
    print(f"\nOutput directory: {OUTPUT_DIR / ANIMATION_STYLE['id']}")
    print("\n" + "="*60)
    ...
    style_dir = OUTPUT_DIR / ANIMATION_STYLE['id']
    style_dir.mkdir(parents=True, exist_ok=True)

    # Generate the full prompt
    full_prompt = f"{ANIMATION_STYLE['example']}, {ANIMATION_STYLE['prompt']}"
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i, url in enumerate(image_urls, 1):
                filename = f"{ANIMATION_STYLE['id']}_preview_{i}.png"
    ...
    summary_file = style_dir / f"{ANIMATION_STYLE['id']}_urls.txt"
    with open(summary_file, 'w') as f:
        f.write(f"Animation (Disney-Pixar) Style Preview URLs\n")
